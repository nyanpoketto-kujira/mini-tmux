...
- Verifikasi tanpa menjalankan bot: `node --check index.js` + ekstrak fungsi asli (`escapeBlock`, `cleanAnsi`, `renderBlock`, `renderPage`, `splitChunk`, `flushSession`) ke `vm` — lihat sim di `/tmp/opencode/sim.js` (mode `main`/`unit`/`rate`).
